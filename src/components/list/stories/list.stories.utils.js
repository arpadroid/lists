...
import { attrString } from '@arpadroid/tools/html';
 ...
 */
export async function playSetup(canvasElement, initList = true, preRenderCallback) {
...
/**
 * Gets the default arg types for the list story.
 * @param {string} [category]
 * @returns {import('@storybook/web-components-vite').ArgTypes}
 */
export function getArgTypes(category = 'List Props') {
    return {
        ////////////////////////
        // General
        ////////////////////////
        ...
        ////////////////////////
        controls: {
            description: html`The list of controls to display, defined as a comma-separated list, they map to different
                UI components. They are all enabled by default and can be sorted as per definition e.g.
        ...
        hasInfo: {
            ...
            page, and search results. It also includes a refresh button to reload the list, and previous and next page
            buttons if hasPager is enabled.`,
            control: { type: 'boolean' },
            table: {
        ...
        ///////////////////////////////
        hasResource: {
            description: html`If true it uses a resource class to manage the list items and associated data. These can
            be fetched from a URL configured via the url attribute or handled statically via the addItems and similar
            methods in the list component. The resource class can be interfaced with via the listResource property in
            the list component or the list component's public methods.`,
            control: { type: 'boolean' },
            table: {
                defaultValue: { summary: 'false' },
                category,
        ...
        url: {
            description: html`The URL used by the list resource to fetch the list items. It is not required to define a
            URL for static lists where items are added manually via the list component.`,
            control: { type: 'text' },
            table: { category, subcategory: 'Resource' }
        },
        paramNamespace: {
            description: html`The namespace used to prefix the query parameters for the list filters. List filters are
            dynamic and can be created, configured and added to the list, which managed via the listResource.`,
            control: { type: 'text' },
            table: { category, subcategory: 'Resource' }
        }
        ///////////////////////////////
